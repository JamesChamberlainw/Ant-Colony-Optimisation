...
def check_cdf(cdf_row, rand):
    ...
    returns: int index of the cdf_row
    """
    for i in range(len(cdf_row)):
        if rand < cdf_row[i]:
            return i

    return -1 # if -1 there has been a problem and a error has occured (should never happen) 
...
def cdf_generate(pheromone_row, huristic_row, alpha, beta):
    ...
    # convert to cumulative distribution function (cdf) for each row of the matrix 
    cdf = combined_matrix / combined_matrix.sum(axis=1)[:, None]
